    ...
    public class Solution
    {
        ...
        public int solution(int[] X, int[] Y)
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                var p0 = points[i];
                used.Add(p0.i);
                var ppp = points.Where(t => t.i !=i);
                foreach (var p1 in ppp.Where(p1 => (p1.x == p0.x) && Math.Abs(p1.y - p0.y) % 2 == 0))
                {
                    foreach (var p2 in ppp.Where(p2 => (2 * p2.y == p0.y + p1.y) && p2.x > p0.x))
                    {
                        var pp = ppp.FirstOrDefault(p3 => (p3.y == p2.y) && (p3.x + p2.x == 2 * p1.x));
                        if (pp != null)
                        {
                            var p4 = new point4(p0.i, p1.i, p2.i, pp.i);
                            if (!point4s.Any(t => t.i0 == p4.i0 && t.i1 == p4.i1 && t.i2 == p4.i2 && t.i3 == p4.i3))
                            {
                                Console.WriteLine($"{p0.i},{p1.i},{p2.i},{pp.i}");
                                point4s.Add(p4);
                                result++;
                                break;
                            }
                ...
                foreach (var p1 in ppp.Where(p1 => (p1.y == p0.y) && Math.Abs(p1.x - p0.x) % 2 == 0))
                {
                    ...
                    {
                        var pp = ppp.FirstOrDefault(p3 => (p3.x == p2.x) && (p3.y + p2.y == 2 * p1.y));
                        if (pp != null)
                        {
                            var p4 = new point4(p0.i, p1.i, p2.i, pp.i);
                            if (!point4s.Any(t => t.i0 == p4.i0 && t.i1 == p4.i1 && t.i2 == p4.i2 && t.i3 == p4.i3))
