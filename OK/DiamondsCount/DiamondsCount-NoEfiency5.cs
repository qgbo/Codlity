...
namespace DiamondsCount_NoEfiency6
{
    using System;
    ...
    public class Solution
    {
        ...
        public int solution(int[] X, int[] Y)
        {
            ...
            foreach (var p0 in leftCollection)
            {
                used.Add(p0.i);
                // 跑过一个性能了，得分55！
                foreach (var p1 in yGroup.First(g => g.Key == p0.y))
                {
                    if (!used.Contains(p1.i) && (p1.x - p0.x) % 2 == 0)
                    {
                        var vertical = xGroup.FirstOrDefault(g => g.Key * 2 == p0.x + p1.x)?.ToArray();
                        if(vertical!=null)
